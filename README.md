...
Gboard 是为了解决传统 PHP 面板在高并发和大数据量下的性能瓶颈而生的全新一代管理面板，同时我们抛弃了臃肿缓慢的 WebAPI，采用 **gRPC** 进行高性能通信，实现了节点与面板的秒级同步通信。

它不仅继承了 V2board/Xboard 的优秀生态，更引入了 **傻瓜化安装**、**自动环境配置** ，**快速从V2board/Xboard面板等转换数据的能力** 以及 **强大的落地管理**（WireGuard/Socks/HTTP 出口均衡），是大型站点和追求极致性能用户的最佳选择，也更适合小白开机场。
...
### 🛠️ 傻瓜化部署
- **一键环境配置**：运行安装程序即可自动部署 MySQL 和 Redis，无需手动配置。（也可以手动！）
...
### 🛡️ 安全与备份
- **云端备份**：支持直接备份数据库到 Google Drive, Dropbox 等云存储，目前仅实现Google Drive。
- **数据可视化**：后台直观查看备份与系统状态。
- **数据可视化**：后台直直接进行数据恢复。
